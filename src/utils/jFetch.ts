...
const resultCache = new Map<
  ResourceUrl,
  { rawText: Promise<string | undefined>; reponseTime: number }
>();
...
export async function tryFetch(
  ...
  const useStrongCache =
    resultCache.has(key) &&
    Date.now() - resultCache.get(key)!.reponseTime < 2000;
  if (useStrongCache) {
  ...
  try {
    const canUseCache =
      ...
      (options?.cacheFreshTime
        ? Date.now() - resultCache.get(key)!.reponseTime <
          (options.cacheFreshTime ?? 2000)
        : false);
    if (!canUseCache) {
      const response = fetch(input, options);
      resultCache.set(key, {
        rawText: response.then((r) => r.clone()).then((r) => r.text()),
        reponseTime: Date.now(),
      });
      if (!(await response).ok) {
        resultCache.set(key, {
          rawText: Promise.resolve(undefined),
          reponseTime: Date.now(),
        });
        return undefined;
      }
      ...
      resultCache.set(key, {
        rawText: Promise.resolve(rawText),
        reponseTime: Date.now(),
      });
      return rawText;
    } else {
